...
if(isset($_POST["action"]))
{
	...
	if($_POST["action"] == 'fetch_single')
	{
		...
		foreach($result as $row)
		{
			...
			$data['doctor_Education'] = $row['doc_edu'];
			$data['doctor_about'] = $row['doc_about'];
			$data['doctor_interest'] = $row['doc_interest'];
			$data['doctor_award'] = $row['doc_award'];
		}
			...
			if($error == '')
			{
			  ...
			  $encoded = md5($enco);
				...
				$data = array(
					...
					':doctor_degree'				=>	$object->clean_input($_POST["doctor_degree"]),
					':doctor_expert_in'				=>	$object->clean_input($_POST["doctor_expert_in"]),
					':doctor_education'				=>	$object->clean_input($_POST["doctor_Education"]),
					':doctor_award'				=>	$object->clean_input($_POST["doctor_Awards"]),
      				':doctor_interest'				=>	$object->clean_input($_POST["doctor_Interest"]),
					':doctor_about'				=>	$object->clean_input($_POST["doctor_About"])
				...
				$object->query = "
	                ...
	                doc_award  =   :doctor_award,
	                doc_interest = :doctor_interest,
	                doc_about =  :doctor_about,
	                doc_edu  =  :doctor_education
